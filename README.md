...
- `branch_replies`：扩展回复列表；会按顺序判断
- `branch_reply_mode`：扩展回复命中方式，`first` 表示命中第一条就回复，`all` 表示把所有命中的扩展回复都按顺序发出来
- `ignore_case`：是否忽略大小写
...
本版本优化高频关键词命中时的回复速度，避免“记录已回复次数”拖慢实际回复。

#### 优化

- 已回复次数不再在命中关键词后立刻同步写入配置文件
- 改为先在内存里累加，再短延迟合并写回配置
- 插件正常退出时会主动写回尚未落盘的计数
- 最大回复次数仍会立刻按内存计数生效，避免高频触发时突破限制

### v1.4.6

本版本新增“扩展回复命中全部”的能力，适合一条消息里同时包含多个关键词时连续回复多条内容。

#### 新增

- 每条关键词规则新增 `branch_reply_mode`
- `first`：保持旧行为，只回复第一条命中的扩展回复
- `all`：把所有命中的扩展回复按顺序逐条发送
...
并把 `branch_reply_mode` 设成 `all`，那么：

- 消息里只有 `14`：回复一条 `14`
- 消息里只有 `19`：回复一条 `19`
- 消息里同时有 `14` 和 `19`：会连续回复两条，先 `14`，再 `19`

### v1.4.5

本版本修复“新增扩展回复后点保存，前端直接报错或看起来像没反应”的问题。

#### 修复

- 修正扩展回复计数和保存校验时的节点选择器，不再把页头的“扩展回复 X 条”徽章误当成真正的扩展回复表单
- 去掉会把旧页面状态重新带回来的前端本地恢复逻辑，保存和刷新都直接以服务端状态为准
- 保留前端保存异常提示，前端出错时会直接显示具体错误，而不是静默失败

### v1.4.4

本版本继续加固扩展回复的保存流程，避免保存成功后页面又像“没存住”。

#### 修复

- 保存接口在返回成功前，会额外校验规则数和扩展回复数是否与本次提交一致
- 前端会记住刚保存的最新规则；如果服务端马上回读出的内容不一致，会优先保留本地最新版本并提示你再保存一次
- 保存后会自动做一次短延迟校验，尽早暴露“看起来保存了、其实又丢了”的情况
